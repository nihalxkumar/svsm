...
mod tests {
    ...
    // --- AttestationError → SvsmError conversion ---

    #[test]
    fn test_attestation_error_converts_to_svsm_error() {
        let err: SvsmError = AttestationError::UnsupportedTee.into();
        assert!(matches!(err, SvsmError::TeeAttestation(AttestationError::UnsupportedTee)));

        let err: SvsmError = AttestationError::Failed.into();
        assert!(matches!(err, SvsmError::TeeAttestation(AttestationError::Failed)));

        let err: SvsmError = AttestationError::ProxyRead.into();
        assert!(matches!(err, SvsmError::TeeAttestation(AttestationError::ProxyRead)));
    }
    ...
    fn test_hash_empty_params() {
        ...
        // With no params, the hash is SHA-512 of empty input
        let expected = Sha512::digest([]);
        assert_eq!(result, expected.as_slice());
    }

    #[test]
    fn test_hash_challenge_only() {
    ...
    #[test]
    fn test_hash_ec_public_key_only() {
    ...
    #[test]
    fn test_hash_challenge_then_ec_key() {
    ...
    #[test]
    fn test_hash_ec_key_then_challenge() {
    ...
    fn test_hash_returns_64_bytes() {
        ...
        let result = hash(&response, &pub_key).unwrap();
        assert_eq!(result.len(), 64); // SHA-512 output
    }
    ...
    #[test]
    fn test_hash_different_order_produces_different_result() {
